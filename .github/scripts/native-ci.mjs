    ...
    "commands": [
      ...
      },
      {
        ...
        "args": [
          "--input-type=module",
          "-e",
          "import { readdirSync } from 'node:fs'; import { execFileSync } from 'node:child_process'; const wheels = readdirSync('dist').filter(name => name.endsWith('.whl')); if (wheels.length !== 1) throw new Error('Expected one built wheel'); execFileSync('uv', ['pip', 'install', '--python', '.validation-consumer/bin/python', 'dist/' + wheels[0]], { stdio: 'inherit' });"
        ]
      },
      {
        "stage": "import",
      ...
      },
      {
        ...
        "executable": "$SDK_ROOT/.validation-consumer/bin/python",
        "args": [
          "$SDK_ROOT/.github/scripts/validate-python-operation-snippets.py",
          "$SDK_ROOT"
        ]
      },
      {
